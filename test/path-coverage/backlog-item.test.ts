...
describe('Backlog Item Path Coverage Tests', () => {
  ...
  beforeEach(() => {
    user = new User('Luc', 'lhajoost@avans.nl', ScrumRole.SCRUM_MASTER, [new NotificationPreference(NotificationType.SLACK, 'lhajoost@avans.nl')]);
    backlogItem = new BacklogItem('BI-001', 'Make a new Functionality', 'Adding new Functionality to the back-end', user);
    activity = new Activity('Testing', 'Testing the activity of backlog item');
    thread = new Thread('Testing Thread', user); observer = {
      update: jest.fn(),
    };
  ...
  it('should set developer and tester', () => {
    const developer = new User('Developer 1', 'dev@gmail.com', ScrumRole.DEVELOPER);
...
describe('Backlog Item Doing State Tests', () => {
  ...
  it('should not move to doing state, should throw error', () => {
    expect(() => { backlogItem.doing() }).toThrowError();
  });

  it('should move to ready for testing state', () => {
    backlogItem.readyForTesting();
    expect(backlogItem.getState().constructor.name).toBe('BacklogReadyForTestingState');
  });
