...
describe("Logger Tests", () => {
  ...
  const productOwner: User = new UserFactory().createUser(
    "Erdem",
    "Pekguzel",
    "er@d.em",
    "0612345678",
  ...
  const scrumMaster: User = new UserFactory().createUser(
    "Erdem",
    "Pekguzel",
    "er@d.em",
    "0612345678",
  ...
  it("should output a simple log", () => {
    ...
    expect(simpleLogVisitor.getLog()).toBe(
      `${pipelineTestState.getName()}: ${pipelineTestState.getAction()}`
    );
  });
  ...
  it("should output an advanced log", () => {
    ...
    expect(advancedLogVisitor.getLog()).toBe(
      `Currently processing the following action: ${pipelineTestState.getAction()} in the ${pipelineTestState.getName()}!`
    );
  });
});
