...
describe('User', () => {
  let user: User;
  beforeEach(() => {
    user = new UserFactory().createUser('Erdem', 'Pekguzel', 'lhajoost@avans.nl', '0637916452', new Array<NotificationPreference>(), ScrumRole.DEVELOPER);
  });

  describe('constructor', () => {
    it('should create a user with notification preference', () => {
      user.addNotificationPreference(NotificationType.SLACK, 'lhajoost@avans.nl')
      ...
      expect(user.getNotificationPreferences()).toEqual([{ _type: NotificationType.SLACK, _address: 'lhajoost@avans.nl' }]);
    });
  ...
  describe('setters', () => {
    it('should update user information', () => {
      const user = new UserFactory().createUser('Erdem', 'Pekguzel', 'lhajoost@avans.nl', '0637916452', new Array<NotificationPreference>(), ScrumRole.DEVELOPER);
  ...
  describe('addNotificationPreference', () => {
    it('should add a notification preference', () => {
      user.addNotificationPreference(NotificationType.EMAIL, 'lhajoost@avans.nl');
      user.addNotificationPreference(NotificationType.SLACK, 'lhajoost@avans.nl');
      expect(user.getNotificationPreferences()).toEqual([{ _type: NotificationType.EMAIL, _address: 'lhajoost@avans.nl' }, { _type: NotificationType.SLACK, _address: 'lhajoost@avans.nl' }]);
    });
  });
  ...
  describe('removeNotificationPreference', () => {
    it('should remove a notification preference', () => {
      ...
      console.warn('SHOULD HAVE EMPTY ARRAY: ' + user.getNotificationPreferences())

      user.addNotificationPreference(NotificationType.EMAIL, 'lhajoost@avans.nl');
      user.addNotificationPreference(NotificationType.SLACK, 'lhajoost@avans.nl');

      console.warn('SHOULD HAVE TWO ITEMS IN ARRAY: ' + user.getNotificationPreferences())
      let notificationPreference = new NotificationPreference(NotificationType.EMAIL, 'lhajoost@avans.nl');
      user.removeNotificationPreference(notificationPreference);

      console.warn('SHOULD HAVE ONE ITEM IN ARRAY: ' + user.getNotificationPreferences())
